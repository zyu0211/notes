...
1. 镜像下载，地址：[https://archlinux.org/download/](https://archlinux.org/download/)
2. 使用Ventoy制作U盘启动，下载地址：[https://www.ventoy.net/cn/download.html](https://www.ventoy.net/cn/download.html)
3. 将下载好的镜像拷贝到启动盘中。
...
```
pacstrap /mnt base base-devel linux linux-firmware linux-headers networkmanager nano grub os-prober efibootmgr ntfs-3g amd-ucode bluez bluez-utils
```
...
设置这两个文件：`locale.gen` 与 `locale.conf`。

1. 编辑 `/etc/locale.gen`，取消 `en_US.UTF-8 UTF-8`  和 `zh_CN.UTF-8 UTF-8` 注释。
...
3. 创建 locale.conf 文件，并 编辑设定 LANG 变量：
...
如果是双系统，需要启用 os-prober 发现其他操作系统。编辑 grub 配置文件
